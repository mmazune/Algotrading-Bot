...
def test_lsg_smoke():
    """
    Smoke test: run LSG on 20-30 days of EURUSD 5m data with auto provider.
    """
    symbol = "EURUSD"
    interval = "5m"
    days = 30
    ...
    if 'debug' in metrics:
        debug = metrics['debug']
        print(f"\n✓ LSG smoke test passed: {metrics['trade_count']} trades over {days} days")
        print(f"  Total return: {metrics['total_return']*100:.2f}%")
        print(f"  Provider: {provider.last_source_used}")
        print(f"  Debug: clusters_high={debug.get('clusters_high', 0)}, "
              f"clusters_low={debug.get('clusters_low', 0)}, "
              f"sweeps_high={debug.get('sweeps_high', 0)}, "
              f"sweeps_low={debug.get('sweeps_low', 0)}, "
